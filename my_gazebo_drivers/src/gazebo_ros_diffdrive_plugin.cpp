...
GZ_REGISTER_DYNAMIC_CONTROLLER("gazebo_ros_diffdrive_plugin", DiffDrivePlugin);
  ...
  Controller(parent)
{
  ...
  topicNameP = new ParamT<std::string> ("topicName", "cmd_vel", 0);
  Param::End();
...
void DiffDrivePlugin::LoadChild(XMLConfigNode *node)
{
  ...
  // Initialize gazebo ROS node if not already initialised
  ...
  //Read namespace from spawn_model
  //Node namespace already fixed (usually /, since gazebo node is usually there)
  //This namespace is suggested by spawn_model
  //Default given on creation of parameter (never used)?
  //Overridden by namespace of spawn_model (e.g. / or /my_robot/)?
  //Overridden by "-namespace" command line argument to spawn_model?
  //Overridden by XML property in model?
  robotNamespaceP->Load(node);
  robotNamespace = robotNamespaceP->GetValue();

  //Read topic from spawn_model
  //This topic is suggested by spawn_model
  //Default is given on creation of parameter?
  //Overridden by XML property in model?
  topicNameP->Load(node);
  topicName = topicNameP->GetValue();

  //Create virtual node name
  std::string nodeName = "gazebo_ros_diffdrive_plugin";

  //Create node handle
  rosnode_ = new ros::NodeHandle(robotNamespace);
  std::string nodeNamespace = rosnode_->getNamespace();

  //Create keys for parameters
  std::string commandTopicKey = nodeNamespace + "/" + nodeName + "/" + "command_topic";
  std::string odomTopicKey = nodeNamespace + "/" + nodeName + "/" + "odom_topic";

  //Read parameters, setting default values if needed, and advertising
  std::string commandTopic;
  std::string odomTopic;
  rosnode_->param(commandTopicKey, commandTopic, nodeNamespace + "/" + nodeName + "/" + topicName);
  rosnode_->param(odomTopicKey, odomTopic, nodeNamespace + "/" + nodeName + "/" + "odom");
  rosnode_->setParam(commandTopicKey, commandTopic);
  rosnode_->setParam(odomTopicKey, odomTopic);
  ...
  ros::SubscribeOptions so = ros::SubscribeOptions::create<geometry_msgs::Twist>(
      commandTopic, 1, boost::bind(&DiffDrivePlugin::cmdVelCallback, this, _1), ros::VoidPtr(), &queue_);
  sub_ = rosnode_->subscribe(so);
  pub_ = rosnode_->advertise<nav_msgs::Odometry>(odomTopic, 1);
}
...
void DiffDrivePlugin::UpdateChild()
{
  ...
  d1 = stepTime.Double() * wd / 2 * joints[RIGHT]->GetVelocity(0);
  d2 = stepTime.Double() * wd / 2 * joints[LEFT]->GetVelocity(0);
...
void DiffDrivePlugin::publish_odometry()
{
  ...
  qt.setEuler(pos_iface_->data->pose.yaw, pos_iface_->data->pose.pitch, pos_iface_->data->pose.roll);
